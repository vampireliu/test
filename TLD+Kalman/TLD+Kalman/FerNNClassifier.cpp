...
void FerNNClassifier::read(const FileNode& file){
  ...
  nstructs = (int)file["num_trees"]; //10 树木（由一个特征组构建，每组
                                     //特征代表图像块的不同视图表示）的个数  
  structSize = (int)file["num_features"]; //13  每棵树的特征个数，也即每棵树的
                                          //节点个数；树上每一个特征都作为
                                          //一个决策节点  
...
void FerNNClassifier::prepare(const vector<Size>& scales)//prepare的工作就是给每一个扫描窗口初始化对应的pixel comparisons（俩个随机分配的
														//像素点坐标），然后初始化后验概率为0；
{
  acum = 0;
  //Initialize test locations for features
  int totalFeatures = nstructs*structSize;//10*13//这里的参数由parameters.yml文件进行初始化，具体是什么意思呢，structSize：13对像素比较
   //二维向量包含全部尺度（scales）的扫描窗口，每个尺度包含totalFeatures个特征  
  features = vector<vector<Feature> >(scales.size(),vector<Feature> (totalFeatures));//vector(最大容量，初始所有值)
  RNG& rng = theRNG();//opencv中自带的一个随机数发生器的类RNG
  float x1f,x2f,y1f,y2f;
  int x1, x2, y1, y2;

  /*
  集合分类器基于n个基本分类器，每个分类器都是基于一个pixel comparisons
  （像素比较集）的;
  pixel comparisons的产生方法：
  先用一个归一化的patch去离散化像素空间，产生所有可能的垂直和水平的pixel comparisons 
  然后我们把这些pixel comparisons随机分配给n个分类器，每个分类器得到完全不同的
  pixel comparisons（特征集合），这样，所有分类器的特征组统一起来就
  可以覆盖整个patch了
  用随机数去填充每一个尺度扫描窗口的特征 
  */
  ...
  /* 初始化后验概率
  1、后验概率是指每一个分类器对传入的图像片进行像素对比，每一个像素对比得到
  0或者1，所有的特征共有13个comparison对比，连成一个13位的二进制代码x，然后
  索引到一个记录了后验概率的数组p(y|x)，y为0或者1（二分类）也就是出现x的基础
  上，该图像片为y的概率是多少，对n个基本分类器的后验概率做平均，大于0.5的则
  判定其含有目标。
  */
  for (int i = 0; i<nstructs; i++) 
  {
	  /*
	  每一个分类器维护一个后验概率的分布，这个分布有2^d个条目，这里的d是像素
	  比较pixel comparisons的个数，这里的structSize,即13个comparision，所以会
	  产生2^13即8192种可能的code，每个code对应一个后验概率。
	  后验概率p(y|x)=#p/(#p+#n),#p和#n分别表示正负图相片的数目，也就是下面的
	  pCounter和nCounter
	  初始化时，每个后验概率都得初始化为0；运行时候以下面的方式更新：已知类别
	  标签的样本（训练样本）通过n个分类器进行分类，如果分类结果错误，那么相应
	  的#p和#n就会更新，这样P(y|x)也相应的更新了。
	  */
...
void FerNNClassifier::trainF(const vector<std::pair<vector<int>,int> >& ferns,int resample){
  ...
  thrP = thr_fern*nstructs;     //0.5*10                                                     // int step = numX / 10;
  //for (int j = 0; j < resample; j++) {                      // for (int j = 0; j < bootstrap; j++) {
	  ...
	  {               //   for (int i = 0; i < step; i++) {
		  ...
		  {                           //       if (Y[I] == 1) {
			  ...
			  该累加值如果小于正样本阈值，也就是是输入的是正样本，却被分类
			  成负样本了
...
void FerNNClassifier::trainNN(const vector<cv::Mat>& nn_examples){
  float conf,dummy;
  vector<int> y(nn_examples.size(),0);//vector<T> v3(n, i); v3包含n个值为i的元素。
                                      //y数组元素初始化为0  
  ...
  for (int i=0;i<nn_examples.size();i++){                          //  For each example
	  ...
	  //标签是正样本，如果相关相似度小于0.65 ，
	  //则认为其不含有前景目标，也就是分类错误了；
	  //这时候就把它加到正样本库 
...
void FerNNClassifier::NNConf(const Mat& example, vector<int>& isin,float& rsconf,float& csconf){
  ...
  /*
  比较图像片p到在线模型M的距离（相似度），计算正样本最近邻相似度，
  也就是将输入的图像片与在线模型中所有的图像片进行匹配，找出最相似
  的那个图像片，也就是相似度的最大值  
  */
...
float fconf;
  for (int i=0;i<nXT.size();i++){
	...
	measure_forest返回的是所有后验概率的累加和，nstructs 为树的个数，也
	就是基本分类器的数目 ？？
	*/
